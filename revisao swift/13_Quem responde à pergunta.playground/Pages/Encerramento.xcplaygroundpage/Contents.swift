/*:
 ## Encerramento
 Você trabalhou muito para criar um cérebro para o QuestionBot e viu como os playgrounds podem ser úteis para trabalhar em uma função sem distrações.
 
 A parte final da aula é colocar no app a nova função que você criou na página anterior.
 
 As instruções para isso estão no seu guia, na aula QuestionBot.*/
/*:
